...
const TransactionItem = ({transaction,display,index}) => {
  ...
  return (
    <div key = {index} className = "w-full my-6" >
      ...
      <div className = {`flex-col-reverse items-start gap-y-2 flex sm:items-center sm:justify-between px-4 py-3 ${handleAmount(transaction.amount) ? 'bg-red-400 bg-opacity-[3%]' : 'bg-green-200 bg-opacity-[3%]'} rounded-xl`}>
        <h1 className = "flex-1 text-xs font-medium text-white sm:text-sm">{transaction.merchant_name || transaction.name}</h1>
